    ...
    fun setDrivePowers(powers: PoseVelocity2d) {
        when (type) {
            DriveType.MECANUM -> {
                ...
                leftMotors[0].power = wheelPowers.leftFront.value() / divisor* 0.99
                leftMotors[1].power = wheelPowers.leftBack.value() / divisor
                rightMotors[0].power = wheelPowers.rightFront.value() / divisor*0.975
                rightMotors[1].power = wheelPowers.rightBack.value() / divisor*0.98
            }
